	...
	for (size_t i = 0; i < NN; i++) {
		...
		if ((int) validOp / 10 == 1) {
			...
			if (opVal == NONE) {
				...
				// PWB(&ANN(rfc, i, validOp)->epoch);  // needed if there is a chance that epoch will be persisted but val not
	...
	if (globalRecovery) {
		...
		for (int i=0; i<NN; i++) { 
			...
			if ((int) validOp / 10 == 0) { // if not valid - make it valid, i.e. allow the combiner to collect
				rfc->announce_arr[i]->valid = 10 + (int) validOp;
			}
			size_t opEpoch = ANN(rfc, i, validOp)->epoch;
			if (opEpoch == rfc->cEpoch) { 
				ANN(rfc, i, validOp)->val = NONE;
			}
		}
		size_t opEpoch = rfc->cEpoch;  // this is important for cases in which a late-arriving process eventually gets to be a combiner
		combine(rfc, opEpoch, pop, pid);
		gRecoveryLock.store(2, std::memory_order_release);
	}
	else {
		while (gRecoveryLock.load() == 1) {} // Spin until recovery is complete
	}
	return VALID_ANN(rfc, pid)->val;
